...
def 지진파_그리기(data: pd.DataFrame, 
    ...
    """
    # 조건에 맞는 자료 필터링
    filtered = data[data["station"] == 관측소]

    if filtered.empty:
        print(f"해당 조건의 데이터가 없습니다. (관측소: {관측소})")
        return
...
def 관측소_그리기(data: pd.DataFrame, center: Optional[tuple[float, float]] = None, zoom_start: int = 10, show_station_labels: bool = True):
    """
...
def 인공지능_그리기(
    ...
    # 대상 관측소만 필터
    filtered = data.loc[data["station"] == 관측소].copy()
    if filtered.empty:
        print(f"[warn] station='{관측소}' 에 해당하는 행이 없습니다.")
        return
...
if __name__ == "__main__":
    ...
    # 데이터 불러오기
    data = 불러오기(args.data, verbose=True)

    # 인공지능 모델로 P/S 도달시각 결정
    data_rel = 인공지능모델(data, model=args.model, verbose=True)

    # 역산으로 진원 결정
    result_df = 진원결정(data_rel, iteration=args.iter)
